/*  Time of Flight for 2DX4 -- Studio W8-1
                ...
                The VL53L1X is run with default firmware settings.
								...
								This code is updated and expanded upon for the final project.
...
while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R9) == 0){}; //allow time for clock to stabilize
	GPIO_PORTK_DIR_R = 0b00011111;// Make PK0:PK3 outputs, controlling the stepper motor 
	GPIO_PORTK_DEN_R = 0b00011111;
return;
}
...
int main(void) {
  ...
  uint8_t dataReady;
	...
	int mynumber = 1;
	sprintf(printf_buffer,"2DX4 Final Project Program %d\r\n",mynumber);
...
//  status = VL53L1X_SetInterMeasurementInMs(dev, 200); /* in ms, IM must be > = TB */
	...
	while(1){
		if(state==1){
			...
			while(step<512){//one full revolution clockwise
				//clockwise 4 steps
